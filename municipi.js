    ...
    promises.push(
      fetch(`./boundaries/geoJson/municipi_${i}.geojson`)
        ...
        .then((data) => {
          L.geoJSON(data, {
            style: (feature) => ({
              fillColor: getMunicipioColor(feature.properties.municipio),
              weight: 3,
              opacity: 1,
              color: "#333333",
              fillOpacity: 0.1,
            ...
            onEachFeature: (feature, layer) => {
              ...
              const center = layer.getBounds && layer.getBounds().getCenter ? layer.getBounds().getCenter() : null;
              const coordLine = center ? `Lat: ${center.lat.toFixed(5)}, Lng: ${center.lng.toFixed(5)}` : '';
              layer.bindPopup(
  ...
  if (view === "full") {
    municipioLayers.forEach((layer) => {
      ...
      layer.setStyle({
        ...
        color: "#333333",
        fillOpacity: 0.1,
        fillColor: getMunicipioColor(layer.municipioId),
      });
      const c1 = layer.getBounds && layer.getBounds().getCenter ? layer.getBounds().getCenter() : null;
      const c1Text = c1 ? `Lat: ${c1.lat.toFixed(5)}, Lng: ${c1.lng.toFixed(5)}` : '';
      layer.bindPopup(
    ...
    municipioLayers.forEach(layer => {
      ...
      if (Number(layer.municipioId) === selectedId) {
        ...
        const c2 = layer.getBounds && layer.getBounds().getCenter ? layer.getBounds().getCenter() : null;
        const c2Text = c2 ? `Lat: ${c2.lat.toFixed(5)}, Lng: ${c2.lng.toFixed(5)}` : '';
        layer.bindPopup(
          `<h3>Municipio ${layer.municipioData.numero_rom}</h3>
            <div class=\"municipio-info\">\n            <strong>ID:</strong> ${layer.municipioData.municipio}<br>\n            <strong>Municipio:</strong> ${layer.municipioData.numero_rom}\n            </div>
        ...
        if (layer.getBounds && layer.getBounds().isValid && layer.getBounds().isValid()) {
          map.fitBounds(layer.getBounds(), { padding: [80, 80], maxZoom: 13, animate: true });
        } else {
          console.warn("No valid bounds for municipio", layer.municipioId, layer);
        }
...
function onMunicipioClick(e) {
  ...
  document.getElementById("viewSelect").value = layer.municipioData.numero_rom;
  handleMunicipioViewChange();
  if (typeof window.handleHotelViewChange === 'function') window.handleHotelViewChange();
}